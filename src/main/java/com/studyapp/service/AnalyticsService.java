    ...
    public StatisticsResult getStatistics(String userId) {
        ...
        for (Subject subject : subjects) {
            ...
            int wMin = studyLogRepository
                    .findByUserIdAndSubjectIdAndDateAfter(userId, subject.getId(), weekAgo)
                    .stream().mapToInt(StudyLog::getDurationMinutes).sum();
            weekData.add(wMin);

            // 월간 학습 시간
            int mMin = studyLogRepository
                    .findByUserIdAndSubjectIdAndDateAfter(userId, subject.getId(), monthAgo)
                    .stream().mapToInt(StudyLog::getDurationMinutes).sum();
            monthData.add(mMin);
        }
    ...
    public List<WrongAnswerItem> getWrongAnswers(String userId, String subjectId) {
        ...
        return results.stream()
                .map(qr -> new WrongAnswerItem(
                        qr.getId(),
                        qr.getSubjectId(),
                        subjectNameMap.getOrDefault(qr.getSubjectId(), "기타"),
                        qr.getQuestionText(),
                        qr.getUserAnswer(),
                        qr.getCorrectAnswer(),
                        qr.getExplanation(),
